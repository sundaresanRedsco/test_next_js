...
export const authOptions: NextAuthOptions = {
  providers: [
    // Standard Credentials Login Provider
    CredentialsProvider({
      name: "Credentials",
      credentials: {
        email: { label: "Email", type: "email", placeholder: "Email" },
      ...
      async authorize(
        ...
        try {
          ...
          if (result.statuscode !== 200) {
            throw new Error(errorHandling(result || "Login failed"));
          }
    ...
    // (Optional) Add AzureADProvider if needed.
  ],
  secret: process.env.NEXTAUTH_SECRET || "",
  callbacks: {
    // signIn callback: For SSO providers, call your backend with the provider's token and override user tokens.
    async signIn({ user, account }) {
      if (account?.provider === "google" || account?.provider === "github") {
        try {
          ...
          if (result.statuscode !== 200) {
            throw new Error(
              result.errormessage || "Backend token retrieval failed"
            );
          }
          // Override user object tokens with tokens from your backend.
          const { access_token, refresh_token } = result.data;
    ...
    // JWT callback: Merge user info (including backend tokens) into the JWT token.
    async jwt({ token, user }) {
      if (user) {
        Object.assign(token, user);
      }
    ...
    // Session callback: Expose the merged token (user data) in the session.
    async session({ session, token }) {
      if (token) {
        session.user = { ...session.user, ...token };
      }
